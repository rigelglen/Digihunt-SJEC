...
function PuzzleImg(id_p, im, cols, rows, wg, hg) {
  ...
  function setElms(id_p, im) {
    ...
    if (height == '') height = img.height; //add canvas and solve button

    var parent = document.getElementById(id_p);
    id_solv = id_p + '_solv';
    parent.innerHTML = '<canvas id="' + id_p + '_cnv" width="' + width + '" height="' + height + '" class="puzcnv"></canvas>';
    ...
    cnv.addEventListener('click', function (ev) {
      if (solv == 0) {
        ...
        me.clicks++;
        var x = ev.offsetX;
        var y = ev.offsetY; //detect clicked tile from $tl_p
          ...
          if (y > tl_p[id].ty && y < tl_p[id].ty + tl_size.h && x > tl_p[id].tx && x < tl_p[id].tx + tl_size.w) {
            ...
            if (tl_c == -1) {
              ...
              var tl2 = {
                ...
                ty: tl_p[id].ty,
                ord: tl_p[id].ord
              }; //data of 2nd tile to be added in 1st tile
  ...
  function setImP() {
    for (var i = 0; i < cols * rows; ++i) {
      ...
      var r = i % rows; //current column /rom of piece in img
    ...
    for (var j, x, i = im_p.length; i; j = Math.floor(Math.random() * i), x = im_p[--i], im_p[i] = im_p[j], im_p[j] = x) {
      ;
    } //shuffle array
  ...
  function setTL() {
    for (var i = 0; i < im_p.length; i++) {
      var c = Math.floor(i / rows);
      var r = i % rows; //current column /rom of tile in canvas
  ...
  function checkPuzzle() {
    ...
    if (re == 1) {
      cnt.drawImage(img, 0, 0, width, height); //if solved manually (-1 is auto) calls solved()
  ...
  me.delSolve = function () {
    document.getElementById(id_solv).outerHTML = '';
    id_solv = '';
  }; //called when puzzle is completed


  me.solved = function () {
    if (me.clicks > 0) alert('Congratulations, you solved the puzzle in ' + me.clicks + ' clicks');
