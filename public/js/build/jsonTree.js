...
var jsonTree = function () {
  ...
  var utils = {
     ...
     */
    forEachNode: function forEachNode(obj, func) {
     ...
     */
    isValidRoot: function isValidRoot(jsonObj) {
     ...
     * Extends some object
     */
    extend: function extend(targetObj, sourceObj) {
  ...
  function _NodeSimple(label, val, isLast) {
    ...
    var self = this,
        ...
        labelEl,
        template = function template(label, val) {
      var str = '\
                    <span class="jsontree_label-wrapper">\
                        <span class="jsontree_label">"' + label + '"</span> : \
                    </span>\
                    <span class="jsontree_value-wrapper">\
                        <span class="jsontree_value jsontree_value_' + self.type + '">' + val + '</span>' + (!isLast ? ',' : '') + '</span>';
      return str;
    };
  ...
  _NodeSimple.prototype = {
     ...
     * Mark or unmark node
     */
    toggleMarked: function toggleMarked() {
     ...
     *                                (from node to root)
     */
    expandParent: function expandParent(isRecursive) {
      if (!this.parent) {
        return;
      }
     ...
     * @returns {string}
     */
    getJSONPath: function getJSONPath(isInDotNotation) {
      if (this.isRoot) {
        return "$";
      }
  ...
  function _NodeComplex(label, val, isLast) {
    ...
    var self = this,
        el = document.createElement('li'),
        template = function template(label, sym) {
      var comma = !isLast ? ',' : '',
          str = '\
                        <div class="jsontree_value-wrapper">\
                            <div class="jsontree_value jsontree_value_' + self.type + '">\
                                ...
                                <b>' + sym[1] + '</b>' + '</div>' + comma + '</div>';

      if (label !== null) {
        str = '\
                        <span class="jsontree_label-wrapper">\
                            <span class="jsontree_label">' + '<span class="jsontree_expand-button"></span>' + '"' + label + '"</span> : \
                        </span>' + str;
      }
  ...
  utils.extend(_NodeComplex.prototype, {
     ...
     */
    expand: function expand(isRecursive) {
      if (this.isEmpty) {
        return;
      }
     ...
     */
    collapse: function collapse(isRecursive) {
      if (this.isEmpty) {
        return;
      }
     ...
     */
    toggle: function toggle(isRecursive) {
      if (this.isEmpty) {
        return;
      }
     ...
     */
    findChildren: function findChildren(matcher, handler, isRecursive) {
      if (this.isEmpty) {
        return;
      }
  ...
  Tree.prototype = {
     ...
     */
    loadData: function loadData(jsonObj) {
     ...
     */
    appendTo: function appendTo(domEl) {
     ...
     */
    expand: function expand(filterFunc) {
     ...
     */
    toSourceJSON: function toSourceJSON(isPrettyPrinted) {
      if (!isPrettyPrinted) {
     ...
     */
    findAndHandle: function findAndHandle(matcher, handler) {
  ...
  return {
     ...
     * @returns {Tree}
     */
    create: function create(jsonObj, domEl) {
